...
$("#addTrainBtn").on("click", function() {
  ...
  var trnFirst = moment($("#firstTrtainTime").val().trim(), "HH:mm").format("X");
  var trnFreq = $("#frequency-input").val().trim();

  // First Time (pushed back 1 year to make sure it comes before current time)
      var firstTimeConverted = moment.unix(trnFirst).subtract(1, "years"); // find out more about .substract(1 ...)
...
database.ref().on("child_added", function(childSnapshot, prevChildKey) {
  ...
  trnFreq + "</td>   <td>" + nextArrival + "</td><td>" + tMinutesTillTrain + "</td>    </tr>");
